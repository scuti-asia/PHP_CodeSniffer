...
class UpperCaseConstantNameSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (strtolower($tokens[$stackPtr]['content']) === 'define') {
            $constant = $phpcsFile->findNext(Tokens::$emptyTokens, ($stackPtr + 1), null, true);
            $error = 'Declare constants with const. Avoid using define as much as possible; expected const but found define';
            $phpcsFile->addWarning($error, $constant, 'ConstantNotDefine', ['', '']);
            return;
        }
